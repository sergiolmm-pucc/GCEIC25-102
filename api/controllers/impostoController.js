...
exports.getNotaFiscal = (req, res) => {
  ...
  if (!valor_produto || !valor_ipi || !valor_pis || !valor_cofins || !valor_icms) {
    return res.status(400).json({ error: 'Parâmetros obrigatórios não informados' });
  }
  if (isNaN(notaFiscal.valor_produto) || isNaN(notaFiscal.valor_ipi) || isNaN(notaFiscal.valor_pis) || isNaN(notaFiscal.valor_cofins) || isNaN(notaFiscal.valor_icms)) {
